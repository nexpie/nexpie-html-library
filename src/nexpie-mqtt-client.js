/*******************************************************************************
 * Javascript NEXPIE MQTT library for browser
 *
...
/**
 *  Nexgear JS mqtt client
 */

if (typeof Nexgear === "undefined") {
	Nexgear = {};
}
Nexgear.create = function(param) {
  var client_id = param.client_id || "";
	...
	var _nexgear = function(client_id,username) {
  ...
  _nexgear.prototype.subscribe = function(topic,callback){
    ...
    var option={
      onSuccess: function(res){
        response.error=false;
        response.msg=`'${topic}' is subscribed`;
      },
      onFailure: function(err){
        response.error=true;
        response.msg=`can not subscribe '${topic}'`;
      }
    }

    if (self.client && self.client.isConnected()) {
				self.client.subscribe(topic,option);
		}
		else {
      response.error=true;
  ...
  _nexgear.prototype.unsubscribe = function(topic,callback){
    ...
    var option={
      onSuccess: function(res){
        response.error=false;
        response.msg=`'${topic}' is unsubscribed`;
      },
      onFailure: function(err){
        response.error=true;
        response.msg=`can not unsubscribe '${topic}'`;
      }
    }

    if (self.client && self.client.isConnected()) {
        self.client.unsubscribe(topic,option);
    }
    else {
      response.error=true;
  ...
  _nexgear.prototype.publish = function(topic,message,retained,callback){
    ...
    var str_msg = '';
		...
		else if (typeof(message)=='object') str_msg = JSON.stringify(message);
		else str_msg = message.toString();

		var message = new Paho.MQTT.Message(str_msg);
		if (retained) message.retained = true;

		if (self.client && self.client.isConnected()) {
			message.destinationName = topic;
			self.client.send(message);
      response.error=false;
      response.msg=`'topic : ${topic}, message : ${str_msg}' is published`;
		}
		else {
      response.error=true;
  ...
  function _onMessageArrived(res){ //when received message
    self.emit('packetreceive',res)
  }

  function _onMessageDelivered(res){ //when message is send
    self.emit('packetsend',res)
  }

  function _onError(err){ //when error
    self.emit('error',err)
  }

	if (client_id && username) {
		var ng = new _nexgear(client_id,username);
		self = ng;
		return ng;
	}
	else {
